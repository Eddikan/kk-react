...
const Designs = () => {
  ...
  const designsQuery = useGetDesignsQuery(filters);
  ...
  const setDesignss = useSelector((state) => state.designs.designs.data);

  useEffect(() => {
    //   if (designsQuery.data) {
    //     setDesigns(designsQuery.data.data);
    //   }
    designsQuery.refetch();
    console.log("here", setDesignss);
  }, []);
  ...
  const weaves = [
    ...
    "Dobby",
    "Leno",
  ]; // Replace with your array of weave options
    ...
    axios
      ...
      .then((response) => {
        ...
        if (selectedDesigns) {
          ...
          setDesignsLoading(false);
          toast.error(
            "There has been an error getting the portfolio, please try again!"
          );
        }
      })
      .catch((error) => {
                                ...
                                <Slider {...settings}>
                                  ...
                                  selectedCategories.length < 1 ? (
                                    <div className="mx-2 cursor-pointer">
                                      <span class="badge badge-dark bg-dark fs-12 fw-400 text-center">
                                        All
                                      </span>
                                    </div>
                                  ) : (
                                    ...
                                    >
                                      <span class="badge badge-dark bg-white fs-12 text-dark fw-400 text-center">
                                        All
                                      </span>
                                    </div>
                                  )}
                                  {categories.map((category, index) => (
                                    <>
                                      {selectedCategories.includes(
                                        ...
                                        >
                                          <span class="badge badge-dark bg-white text-dark fs-12 fw-400 text-center">
                                            {category.name}
                                          </span>
                                        </div>
                                      )}
                              ...
                              {designs.map((design, index) => {
                                if (design.image_urls?.[0]?.image_url) {
                                  var designImage =
                                    import.meta.env.VITE_REACT_APP_STORAGE_URL +
                                    "portfolio/" +
                                    design.image_urls[0].image_url;
                                } else {
                                  var designImage = DressPlaceholder;
                                }
                                var wishlist_user_ids =
