...
const EditSeller = () => {
    ...
    async function submitProfile(e) {
        ...
        axios.put(import.meta.env.VITE_REACT_APP_API_ENDPOINT + 'user/' + sellerId + '?user_id=' + sellerId + '&current_user_id=' + current_user_id + '&token=' + token, userFormData).then((response) => {
    ...
    async function submitSeller(e) {
        if (areasOfSpecializationData.length > 0) {
            e.preventDefault();
            setUserFormLoading(true);
            axios.put(import.meta.env.VITE_REACT_APP_API_ENDPOINT + 'seller/' + seller.id + '?user_id=' + sellerId + '&current_user_id=' + current_user_id + '&token=' + token, { areas_of_specialization: areasOfSpecializationData }).then((response) => {
                const success = response.data.status;
                if (success == 'Success') {
                    const data = response.data.data;
                    const user = data.user;
