    ...
    async function ProductSubmit(e) {
        ...
        if (productData.image_urls == '') {
            ...
            if (size == "small") {
                ...
                if (productData.image_urls) {
                    setProductLoading(true);
                    axios.post(import.meta.env.VITE_REACT_APP_API_ENDPOINT + 'product?current_user_id=' + current_user_id + '&token=' + token, { ...productData, composition: otherComposition && otherComposition != "" ? otherComposition : composition, weave: otherWeave && otherWeave != "" ? otherWeave : weave, unit_measurement: otherUnitMeasurement && otherUnitMeasurement != "" ? otherUnitMeasurement : unitMeasurement, colors: colors, sustainability: selectedSustainabilities, certifications: certifications, status: 'Active' }).then((response) => {
                                        ...
                                        <Row>
                                            <Col lg="6">
                                                <Form.Group className='my-1'>
                                                    ...
                                                    <FormControl type='number' min = '1' name='price' value={productData.price} className='mr-sm-2' onChange={handleChange} placeholder='' required />
                                                </Form.Group>
                                            </Col>
                                            <Col lg="6">
                                                <Form.Group className='my-1'>
                                                    <Form.Label>Stock Quantity</Form.Label>
                                                    <FormControl type='number' min = '1' name='quantity' value={productData.quantity} className='mr-sm-2' onChange={handleChange} placeholder='' required />
                                                </Form.Group>
                                            </Col>
                                        </Row>
                                    </Card.Body>
