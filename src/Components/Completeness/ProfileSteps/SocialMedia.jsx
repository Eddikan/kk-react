...
const SocialMediaStep = ({ user, currentUser, reload, token }) => {
    ...
    async function submitProfile(e) {
        e.preventDefault();
        setFormStatus(true);
        axios.put(process.env.REACT_APP_API_ENDPOINT + 'user/' + currentUser + '?user_id=' + currentUser + '&token=' + token, { ...profileFormData, profile_completeness: 100 }).then((response) => {
    ...
    async function submitBack(e) {
        e.preventDefault();
        axios.put(process.env.REACT_APP_API_ENDPOINT + 'user/' + currentUser + '?user_id=' + currentUser + '&token=' + token, { profile_completeness: 50 }).then((response) => {
            const success = response.data.status;
            if (success == 'Success') {
                reload();
            } else {
